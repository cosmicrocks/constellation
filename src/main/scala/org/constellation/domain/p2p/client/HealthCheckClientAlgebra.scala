...
trait HealthCheckClientAlgebra[F[_]] {
  def sendPeerHealthStatus[A <: SendConsensusHealthStatus[_, _, _] : Encoder](healthStatus: A): PeerResponse[F, Either[SendProposalError, Unit]]

  def fetchPeerHealthStatus[A <: SendConsensusHealthStatus[_, _, _] : Decoder](fetchPeerHealtStatus: FetchPeerHealthStatus): PeerResponse[F, Either[FetchProposalError, A]]
