...
import org.constellation.domain.redownload.{MissingProposalFinder, RedownloadService}
...
class MissingProposalHealthCheckConsensusManager[F[_]](
  ...
  val stuckMajorityPatience: FiniteDuration = 5.minutes
  ...
  override def periodicOperation(): F[Unit] =
    for {
      ...
      latestMajority <- redownloadService.latestMajorityHeight
      created <- redownloadService.getCreatedSnapshots()
  ...
  override def periodicPeersHealthCheck(): F[Unit] =
    for {
      ...
      proposals <- redownloadService.getAllPeerProposals()
      created <- redownloadService.getCreatedSnapshots()
      ...
      keys = maybeLatestMajorityWithTimestamp match {
        case Some(majorityWithTimestamp)
          ...
          val keys = missing.keySet.map(MissingProposalHealthCheckKey(_, nextHeight)) -- recentlyCheckedKeys
  ...
  override def checkHealthForPeer(key: MissingProposalHealthCheckKey): F[Fiber[F, MissingProposalHealthCheckStatus]] =
    F.start {
      for {
        ownProposals <- redownloadService.getCreatedSnapshots()
        peerProposals <- redownloadService.getAllPeerProposals()
      ...
      case (key, (_, consensus)) =>
        for {
          ...
          _ <- if (ownStatus.isInstanceOf[GotPeerProposalAtHeight])
            ...
            retrieveMissingProposal(key, consensus).handleErrorWith(
              e =>
                logger
                  .warn(e)(
                    s"Failed while retrieving missing proposal at height=${key.height} for id=${logIdShort(key.id)}"
                  )
            )
        } yield ()
    }.void

  private def retrieveMissingProposal(
    ...
    for {
      _ <- logger.debug(s"Retrieving missing proposal for key=${logHealthCheckKey(key)}")
      roundPeers <- consensus.getRoundPeers()
      peersWithMissingProposal = roundPeers.collect {
        case (id, RoundData(_, Some(MissingProposalHealthStatus(_, _, _, _, GotPeerProposalAtHeight(_, _), _)), _)) =>
          id
      }.toList.take(1)
      // TODO: take all peers from above and try until first success instead of taking only one
      _ <- logger.debug(
        s"Found following peers=${logIds(peersWithMissingProposal.toSet)} that may have the missing proposal key=${logHealthCheckKey(key)}"
      )
      _ <- peersWithMissingProposal.traverse(
        redownloadService.fetchProposalForPeer(key.id)(_)
      )
    } yield ()
