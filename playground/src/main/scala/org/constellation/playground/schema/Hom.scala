...
import cats.{Always, Applicative, Eval, Monoid, MonoidK, Traverse}
...
object Hom {
  implicit val traversableInstance: Traverse[Hom] = new Traverse[Hom] {
    override def traverse[G[_], A, B](fa: Hom[A])(f: A => G[B])(implicit G: Applicative[G]): G[Hom[B]] =
      foldRight(fa, Always(G.pure(fiberMonoid.empty[B]))) { (a, acc) =>
        G.map2Eval(f(a), acc) { (b, acc) =>
          fiberMonoid.combineK(acc, acc.coalgebra.run(b))
        }
      }.value
  ...
  implicit val fiberMonoid: MonoidK[Hom] = new MonoidK[Hom] {
    ...
    override def combineK[A](x: Hom[A], y: Hom[A]): Hom[A] = x.tensor(y) //new Hom[A] {}.tensor(x, y)
  }
}
